...
// Add event l
  ...
  faqs.forEach((faq, index) => {
   ...
   // console.log((window.innerWidth-(questionButton.style.width.substring(removePart.length)))+"px")
    ...
    questionButton.style.backgroundColor = "#9c51b6"; // Nice shade of green

    //second-medium
    questionButton.addEventListener('mouseover', () => {
      questionButton.style.backgroundColor = '#c497b3';
    });
    //third-darkest
    questionButton.addEventListener('mousedown', () => {
      questionButton.style.backgroundColor = '#5A164F';
    });
//last-lightest
    questionButton.addEventListener('mouseout', () => {
      questionButton.style.backgroundColor = '#9c51b6';
    });

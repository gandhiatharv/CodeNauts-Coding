...
// Add event l
  ...
  faqs.forEach((faq, index) => {
   ...
   // console.log((window.innerWidth-(questionButton.style.width.substring(removePart.length)))+"px")
    ...
    questionButton.style.backgroundColor = "#e36736"; // Nice shade of green

    //second-medium
    questionButton.addEventListener('mouseover', () => {
      questionButton.style.backgroundColor = '#f5895d';
    });
    //third-darkest
    questionButton.addEventListener('mousedown', () => {
      questionButton.style.backgroundColor = '#D03B00';
    });
//last-lightest
    questionButton.addEventListener('mouseout', () => {
      questionButton.style.backgroundColor = '#e36736';
    });
